...
class EventTest < ActiveSupport::TestCase
  ...
  test "event needs stream" do
    event = Event.new(:starts_at => 1.hour.ago, :ends_at => 1.minute.ago)
    
    assert event.invalid?
  end
  
  test "event needs start date" do
    event = Event.new(:stream => Factory(:stream), :ends_at => 1.minute.ago)
    
    assert event.invalid?
  end
  
  test "event needs end date" do
    event = Event.new(:stream => Factory(:stream), :starts_at => 1.hour.ago)
